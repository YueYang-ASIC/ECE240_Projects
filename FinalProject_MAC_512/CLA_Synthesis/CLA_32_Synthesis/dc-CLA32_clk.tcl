...
#   report_power -hier
#   set_max_delay
#   write -format db -output CLA32_clk.db
#
# Doesn't work quite the way I expect
#   NameDesign = CLA32_clk    Set variable ok, but how to concatenate?
#   write_rtl -format verilog -output CLA32_clk.vg


#===== Set: make sure you change design name elsewhere in this file
set NameDesign "CLA32_clk"
...
analyze -format verilog CLA32_clk.v
analyze -format verilog CLA16.v
analyze -format verilog CLA32.v
...
#===== Reports
write -format verilog -output CLA32_clk.vg -hierarchy $NameDesign
write_sdc	CLA32_clk.sdc
report_area               > CLA32_clk.area
report_power              > CLA32_clk.pow
report_timing -nworst 10  > CLA32_clk.tim
